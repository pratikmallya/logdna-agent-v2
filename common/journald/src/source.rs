...
pub fn create_source(paths: &Vec<PathBuf>) -> impl FutureStream<Item = Vec<LineBuilder>> {
    ...
    let mut streams: Vec<Stream> = journal_directories.into_iter().map(|dir| Stream::new(Path::Directory(dir))).collect();
